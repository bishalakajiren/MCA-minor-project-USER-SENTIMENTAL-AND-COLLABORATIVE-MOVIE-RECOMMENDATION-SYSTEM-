$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'c20d7bbd8fe9a9739ace4a9683a20d84';
    var title = $('.movie').val();
    if (title=="") {
...
// for popularity
function popularity_details(my_api_key,title){
  $.ajax({
    type: 'GET',
    url:'https://api.themoviedb.org/3/movie/popular?api_key='+my_api_key+'&language=en-US&page=1'+'&query='+title,
...
// getting posters for all the recommended movies of popular
function get_movie_popular(arr,my_api_key){
  var arr_popular_list = []
  for(var m in arr) {
    $.ajax({
      type:'GET',
      url:'https://api.themoviedb.org/3/movie/popular?api_key='+my_api_key+'&language=en-US&page=1'+'&query='+arr[m],
      async: false,
      success: function(m_data){
        arr_popular_list.push('https://image.tmdb.org/t/p/original'+m_data.results[0].popular_path);
      },
      error: function(){
        alert("Invalid Request!");
        $("#loader").delay(500).fadeOut();
